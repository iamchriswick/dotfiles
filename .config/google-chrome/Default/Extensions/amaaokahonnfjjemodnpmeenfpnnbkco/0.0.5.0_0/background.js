...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
   ...
   if(request.action === "getAllUserData" ){
     ...
     chrome.storage.sync.get([ 'grepper_user_id','userid', 'access_token', 'email','notices'], function(items){
          sendResponse(items);
      });
     return true;
   }
        ...
        chrome.identity.getProfileUserInfo(function(info) { 
           if(info.email){
                makeRequest('GET', "https://www.codegrepper.com/api/install2.php?is_email=1&u="+info.email).then(function(data){
                    ...
                    setUserData(info.email,newUserData.id,newUserData.access_token,info.email,newUserData.blacklists,newUserData.hide_grepper_button);
                ...
                getChromeUserId().then(function(data){
                    ...
                    var newUserData=JSON.parse(newData);
                        setUserData(data,newUserData.id,undefined,undefined,newUserData.blacklists,newUserData.hide_grepper_button);
                        openWelcomePage(details,newUserData);
                    });
                });
            }
...
//  },5000);
 ...
 function setUserData(chromeUserID,grepperUserID,access_token,email,blacklists,hide_grepper_button){
